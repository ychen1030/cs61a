...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    Sxx = sum([(xs[i] - mean(xs))**2 for i in range(len(xs))])
    Syy = sum([(ys[i] - mean(ys))**2 for i in range(len(ys))])
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    #predictions = dict([(restaurant_name(restaurant), predictor(restaurant)) for restaurant in restaurants])
    predictions = {}
