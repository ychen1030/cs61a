...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if is_prime(points_scored):
        return next_prime(points_scored)
...
def is_perfect_piggy(turn_score):
    ...
    # BEGIN PROBLEM 4
    square, i, cube = 3, 2, 0
    while square < turn_score:
        square = i * i
        cube = square * i
        if square == turn_score or cube == turn_score:
            return True
        i += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if turn_score >= margin or score + turn_score == opponent_score / 2:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    First, choose free bacon if it wins the game;
    Second, try to get a beneficial swap;
    Third, try to enforce beneficial swaps with free bacon;
    Forth, try to prevent harmful swap with free bacon;
    Fifth, rolls 0 dice if it gives at least 8 points.
    ...
    if 2 * (score + turn_score) == opponent_score + 1 or 2 * (score + turn_score) == opponent_score:
        return 0

    if (score + 1 == 2 * (opponent_score + 1) or score + 1 == 2 * opponent_score) and opponent_score > 1:
        return 0
