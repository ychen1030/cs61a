...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if not isinstance(insect, QueenAnt) or not insect.first:
            if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        while new_place.entrance is not hive and distance < self.max_range:
            if new_place.bees and distance >= self.min_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if not self.first:
            ...
            while current:
                if current.ant:
                    if not current.ant in self.doubled:
                        current.ant.damage = current.ant.damage*2
                        self.doubled.append(current.ant)
                    if current.ant.container and current.ant.ant and not (current.ant.ant in self.doubled):
...
def make_slow(action):
    ...
    def count_down(duration):
        def new_act(colony):
            nonlocal duration
            duration -= 1
            if colony.time % 2 == 0 or duration < 0:
